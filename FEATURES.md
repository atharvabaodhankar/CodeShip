# Rovel Features & Capabilities Guide

This document provides a comprehensive breakdown of the features, capabilities, and system polishes implemented in the Rovel Platform-as-a-Service (PaaS). 
...
Rovel automatically inspects the structure of your repository to detect the language and framework being used. It supports:
...
Rovel coordinates with an Nginx reverse proxy and Let's Encrypt Certbot to handle incoming traffic:
* Every project is assigned its own subdomain (e.g., `project-slug.apps.rovel.dev`).
...
To eliminate manual setup, Rovel offers a fully automated GitOps integration:
...
* **Auto-Registration**: Upon project creation, Rovel calls the GitHub Repositories Hook API to automatically register a webhook pointing to the platform's `/api/webhooks/github` endpoint.
...
* **Instant Redeployment**: Every code push triggers a background rebuild automatically. Rovel updates the container, verifies health checks, swaps Nginx routing targets, and prunes the old container with zero downtime.

### 2. Subdirectory & Monorepo Deployments
Rovel supports complex repository layouts, including monorepos and nested project structures:
...
Rovel features a dedicated, secured administration portal designed to give operators complete visibility and control over the platform's resources.
...
To replace clunky native browser selects, Rovel uses a custom-built Combobox:
...
Rovel is architected with a strong focus on platform security:
